...
void displaySkiplist(Skiplist* skiplist) {
    printf("Skiplist levels: %d\n", skiplist->level);
    for (int i = skiplist->level - 1; i >= 0; i--) {
        SkiplistNode *node = skiplist->head->forward[i];
        printf("Level %d: ", i);
        while (node) {
            printf("%d ", node->val);
            node = node->forward[i];
...
int main(){
    ...
    Skiplist* skip_list = skiplistCreate();

    skiplistAdd(skip_list, 234);
    skiplistAdd(skip_list, 18);
    skiplistAdd(skip_list, 198);
    skiplistAdd(skip_list, 387);
    skiplistAdd(skip_list, 237);
    skiplistAdd(skip_list, 188);
    bool is_exist = skiplistSearch(skip_list, 198);
    printf("Is number 198 exist in skip list? [%d]\n", is_exist);
    displaySkiplist(skip_list);

    skiplistErase(skip_list, 18);
    skiplistErase(skip_list, 234);
    skiplistErase(skip_list, 387);
    skiplistErase(skip_list, 188);
    displaySkiplist(skip_list);
    return 0;
}